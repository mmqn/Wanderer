...
const List = ({ places, isMobile, mapboxKey }) => {
	...
	const showCategoryPlaces = selectedCategory => {
					...
					return (
						<Fragment key={category}>
							<div
								...
								role="button"
								onClick={() => showCategoryPlaces(category)}
							>
								<h1>{category}</h1>
							</div>
		...
		<Fragment>
			...
			{isMobile ? (
				<MobileOptionsMenu
					...
					handleCardsDensity={ev => {
						ev.stopPropagation();
						setCondensedCards(!condensedCards);
					}}
					handleCardsStacking={ev => {
						ev.stopPropagation();
				...
				<Fragment>
					...
					<button
						...
						onClick={() => setCondensedCards(!condensedCards)}
					>
					...
					>
						Switch to {stackedView ? "Singled" : "Stacked"} View
					</button>
				</Fragment>
			)}
		</Fragment>
